...
class Editorial(Resource):
    ...
    async def get_selection(self, **kwargs) -> list[Album]:
        """
        Get a list of albums selected every week by the Deezer Team.

        :returns: a list of :class:`Album <deezer.Album>` instances
        """
        return await self.get_relation("selection", **kwargs)

    async def get_chart(self, **kwargs) -> Chart:
        """
        Get top charts for tracks, albums, artists and playlists.

        :returns: a :class:`~deezer.Chart` instance
        """
        return await self.get_relation("charts", resource_type=Chart, **kwargs)

    async def get_releases(self, **kwargs) -> PaginatedList[Album]:
        """
